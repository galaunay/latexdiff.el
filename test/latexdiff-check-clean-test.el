(ert-deftest check-clean ()
  (let* ((file1 (latexdiff-testcase-file1))
    ...
    (let ((diff-file nil)
          ...
          (filepdf nil))
      (with-current-buffer (find-file-noselect file1)
        (setq diff-file (latexdiff-vc--compile-diff-with-current rev1))
        (while latexdiff-runningp (sleep-for 1))
        (setq fileaux (format "%s.out" (file-name-sans-extension diff-file)))
        (setq filetex (format "%s.tex" (file-name-sans-extension diff-file)))
        (setq filepdf (format "%s.pdf" (file-name-sans-extension diff-file)))
        (should (file-exists-p fileaux))
        (should (file-exists-p filetex))
        (should (file-exists-p filepdf)))
      (latexdiff-clean file1)
      (sleep-for 2)
      (with-current-buffer (find-file-noselect file1)
        (should (not (file-exists-p fileaux)))
        (should (not (file-exists-p filepdf)))
